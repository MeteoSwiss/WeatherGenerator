...
class ModelParams(torch.nn.Module):
    ...
    def reset_parameters(self, cf: Config) -> "ModelParams":
        ...
        if cf.get("ae_global_pe_geographic", False):
            # Geographic (lat/lon Fourier) cell identity instead of index-based sinusoids, so
            # that neighbouring cells get similar codes rather than codes that depend on HEALPix
            # ordering. r3tos2 returns azimuth wrapped to (-pi, pi] via atan2; remainder()
            # unwraps it to [0, 2pi) so a domain straddling the wrap stays continuous. The cell
            # centres are recomputed here rather than reused from the rope_2D block above,
            # which only runs when rope_2D is set.
            ...
            # cell, so the finest scale resolves individual cells while the coarsest is smooth
            # over the sphere. dim_embed is split 4 ways: sin/cos x lat/lon.
            n_freq = dim_embed // 4
